...
def save_xml(filepath, panel_data, cad_operations):
    ...
    for op in cad_operations:
        ...
        if op["TypeName"] == "Line":
            parts_x = op.get("X1", "").split(";")
            parts_y = op.get("Y1", "").split(";")
            ET.SubElement(cad, "BeginX").text = parts_x[0] if len(parts_x) > 0 else "0"
            ET.SubElement(cad, "BeginY").text = parts_y[0] if len(parts_y) > 0 else "0"
            ET.SubElement(cad, "EndX").text = parts_x[1] if len(parts_x) > 1 else "0"
            ET.SubElement(cad, "EndY").text = parts_y[1] if len(parts_y) > 1 else "0"
